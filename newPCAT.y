...
declaration: { level++; 
    ...
    | error {printblank(level); printf("Error at (%d, %d). Please check source file.\n", mylineno, mycolumnno); }
    ;
...
procedure_decl : ID {level++; printblank(level); cout << "Procedure " << $1 << " is defined" << endl; level++;
                        ...
                        printblank(level); cout << "Params: " << $3 << endl;
                    } IS body SEMICOLUMN {free($3); free($4); free($6); level-=3;}
    ;
another_procedure_decl: {$$ = (char*)calloc(1, 8); *($$) ='\0';}
    | procedure_decl another_procedure_decl {string res = string($1) + "   " + string($2); $$ = (char*)calloc(res.size() + 1, 8); strcpy($$, res.c_str());}
                                            //free($1); free($2);}
